...
class Order extends ProductCollection implements IsotopePurchasableCollection
{
    ...
    public function checkout()
    {
        ...
        // MARK
        $billing_id = $this->getBillingAddress()->id;

        $obj = Address::findOneBy(['id=?'], [$billing_id], []);
        $groups = $obj->groups;
        ...
        if (\count($notificationIds) > 0) {
            foreach ($notificationIds as $notificationId) {
                ...
                if (($objNotification = Notification::findByPk($notificationId)) !== null) {
                    ...
                    $nTagged = strpos($objNotification->title, '#') === 0;
                    if($nTagged && strpos($objNotification->title, '#'.$notificationId) !== 0) { // if tagged but not with correct id
