...
abstract class OrderConditions extends CheckoutStep
{
    ...
    public function generate()
    {
        ...
        $member_group = \Input::post('billingaddress_groups');

        $business = $this->objModule->iso_order_conditions;
        $private = 4;

        $iso_order_conditions = $member_group == 2 ? $business : $private;

        if ($iso_order_conditions) {
            $objFormConfig = \FormModel::findByPk($iso_order_conditions);

            if (null === $objFormConfig) {
                throw new \InvalidArgumentException('Order condition form "' . $iso_order_conditions . '" not found.');
            }
